...
// DisplayConfig holds display hardware configuration from show_img.json
type DisplayConfig struct {
	Adapter string `json:"adapter"`
	Stretch string `json:"stretch"`
	Panel1  string `json:"panel_1bit"`
	Panel2  string `json:"panel_2bit"`
...
func main() {
	...
	displayConfig := loadDisplayConfig(configDir)
	...
	if options.Verbose {
		fmt.Printf("Display: %dx%d, adapter: %s\n", displayConfig.Width, displayConfig.Height, displayConfig.Adapter)
	}

	frames := 0
	for {
		processNextImage(tmpDir, config, options, displayConfig, frames)
		frames = frames + 1
	}
}
...
func processNextImage(tmpDir string, config Config, options AppOptions, displayConfig DisplayConfig, frames int) {
	...
	req.Header.Add("WIDTH", fmt.Sprintf("%d", displayConfig.Width))
	req.Header.Add("HEIGHT", fmt.Sprintf("%d", displayConfig.Height))
	req.Header.Add("FW_VERSION", version)
	...
	// Display the image
	err = displayImage(filePath, options, displayConfig, frames)
	if err != nil {
...
func displayImage(imagePath string, options AppOptions, displayConfig DisplayConfig, frames int) error {
//
// N.B (Larry Bank)
...
//
        ...
        var err error
        if displayConfig.Adapter == "pimoroni_py" {
              // Use Python display driver for Inky Impression displays not supported by bb_epaper
              // Locate show_image.py next to the trmnl-display binary
              exePath, _ := os.Executable()
              scriptDir := filepath.Dir(exePath)
              scriptPath := filepath.Join(scriptDir, "show_image.py")

              // Fall back to current directory
              if _, statErr := os.Stat(scriptPath); statErr != nil {
                    scriptPath = "show_image.py"
              }

              err = exec.Command("python3", scriptPath,
                    "--file", imagePath,
                    "--invert", invertStr,
                    "--mode", modeStr,
                    "--saturation", "0.5").Run()
              if err != nil {
                    fmt.Printf("show_image.py failed; ensure inky library is installed; error = %v\n", err)
                    os.Exit(0)
              }
        } else {
              // Use bb_epaper show_img for all other display types
              ...
              sb2.WriteString(invertStr)

              sb3.WriteString("mode=")
              sb3.WriteString(modeStr)
...
func loadDisplayConfig(configDir string) DisplayConfig {
	configFile := filepath.Join(configDir, "show_img.json")
	dc := DisplayConfig{Width: 800, Height: 480} // defaults for backward compatibility
	...
	// Default dimensions if not specified in config
	if dc.Width <= 0 {
		dc.Width = 800
	}
	if dc.Height <= 0 {
